...
UTEST(httpp_curl, plain_get_like_client) {
    httpp::server s;
    ...
    httpp::response res = httpp::curl::request(url).run();

    ASSERT_TRUE(res.ok());
    ASSERT_STREQ("world", res.body.c_str());
}

UTEST(httpp_curl, sends_custom_headers) {
    httpp::server s;
    ...
    httpp::response res = httpp::curl::request(url)
                               .header("X-Test", "abc")
                               .run();
...
UTEST(httpp_curl, data_defaults_method_to_post) {
    httpp::server s;
...
UTEST(httpp_curl, explicit_method_overrides_data_default) {
    httpp::server s;
    running_server rs(std::move(s));
...
UTEST(httpp_curl, timeout_and_follow_redirects_are_chainable_and_dont_break_a_request) {
    httpp::server s;
